// This script is used for IF TH neurons identification
...
def TH = display.availableChannels()[1] //define TH channel. the default 1 is red channel.
display.setImageData(imageData, true)
display.setChannelSelected(TH,true)
display.autoSetDisplayRange(TH,0.005)
display.switchToGrayscaleChannelProperty()
//display.setUseInvertedBackground(true)
def test = new ImageDisplaySettings().create("test", 1, false, ['TH'])                    
...
//        .pixelSize( 0.48 )                  // Resolution for detection in um.
          ...
          .setOverlap( 15 )
        .addParameter("no_norm")         // Any parameter from cellpose or omnipose not available in the builder.
